...
const data = async (api, apiType, addressName) => {
    try {
        ...
        if(data) {
            // ★ 서울 ★ 
            let seoul = cityData(data, addressName, "서울");
            console.log(consoleName, "서울", seoul);
            //  ★ 대전 ★ 
            let daejeon = cityData(data, addressName, "대전광역시", "대전");
            console.log(consoleName, "대전", daejeon);
            //  ★ 대구 ★ 
            let daegu = cityData(data, addressName, "대구광역시", "대구");
            console.log(consoleName, "대구", daegu);
            //  ★ 부산 ★ 
            let busan = cityData(data, addressName, "부산광역시", "부산");
            console.log(consoleName, "부산", busan);
            //  ★ 경상도(경남,경북,울산) ★ 
            let gyeongsang = cityData(data, addressName, "경상남도", "경상북도", "울산광역시", "경남", "경북", "울산");
            console.log(consoleName, "경상도", gyeongsang);
            //  ★ 충청도(충남,충북) ★ 
            let chungcheong = cityData(data, addressName, "충청남도", "충청북도", "충남", "충북");
            console.log(consoleName, "충청도", chungcheong);
            //  ★ 제주도 ★ 
            let Jeju = cityData(data, addressName, "제주");
            console.log(consoleName, "제주도", Jeju);
            //  ★ 강원도 ★ 
            let gangwon = cityData(data, addressName, "강원");
            console.log(consoleName, "강원도", gangwon);
            //  ★ 경기도(경기도, 인천, 세종) ★ 
            let gyeonggi = cityData(data, addressName, "경기도", "인천", "세종", "경기");
