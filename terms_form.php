...
회사와 회원 간에 서비스 이용으로 발생한 분쟁에 대하여는 대한민국법을 적용하며, 본 분쟁으로 인하여 소송이 제기될 경우 민사소송법 상의 관할을 가지는 대한민국의 법원에 제기합니다.
            ...
            <div class="agree_box">
                ...
                <label for="agree" class="checkbox-label">약관에 동의합니다</label>
            </div>
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        <td>목적</td>
                        <td>회원제 서비스의 제공을 위한 이용자 식별 및 본인확인 / 부정 가입 및 중복 가입 방지</td>
                    </tr>
                    <tr>
                        <td>항목</td>
                        <td>이름, 아이디, 비밀번호, 이메일, 휴대폰 번호, 기타 번호, 주소, 사업자 등록 번호, 회사명, 회사 대표명, 회사 주소, 회사 팩스 번호, 회사 번호, 업태, 종목</td>
                    </tr>
                    <tr>
                        <td>보유기간</td>
                        <td>회원 탈퇴 후 즉시 파기 처리</td>
                    </tr>
                </tbody>
            </table>
            <h4>※ 개인정보 수집 및 이용에 대한 동의를 거부할 권리가 있으며, 동의 거부 시 회원가입이 제한됩니다.</h4>
            <div class="agree_box">
                <input type="checkbox" id="agree_2"  name="agree_2">
                <label for="agree_2" class="checkbox-label">약관에 동의합니다</label>
            </div>
            <button id="next_btn" type="submit" name="submit">
                다음
            </button>
        </form>
    </main>
    <?php
    // 모든 체크박스에 동의했는지 확인
    $agree_1 = isset($_POST["agree"]) && $_POST["agree"] === "on";
    $agree_2 = isset($_POST["agree_2"]) && $_POST["agree_2"] === "on";

    // 현재 페이지에 대한 요청이 HTTP POST 메서드로 전송되었는지 확인
    if ($_SERVER["REQUEST_METHOD"] == "POST" && $agree_1 && $agree_2) {
        $_SESSION["agreement"] = true;
        echo '<script>window.location.href = "register_form.php";</script>';
        exit;
    } elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
        echo "<script>alert('모든 항목에 동의해야 합니다.');</script>";
    }
    ?>
    <!-- JS -->
